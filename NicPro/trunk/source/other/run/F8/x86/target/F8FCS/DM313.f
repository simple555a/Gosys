...
int main()
{
	...
	if(Force && (Sel_CH & 0x01)){
		...
		A1 = get_rtd_val(pVal[0], pVal[8], T1);
	}
	if(Force && (Sel_CH & 0x02)){
		A2 = FA2;
	}else{
		A2 = get_rtd_val(pVal[1], pVal[8], T2);
	}
	if(Force && (Sel_CH & 0x04)){
		A3 = FA3;
	}else{
		A3 = get_rtd_val(pVal[2], pVal[8], T3);
	}
	if(Force && (Sel_CH & 0x08)){
		A4 = FA4;
	}else{
		A4 = get_rtd_val(pVal[3], pVal[8], T4);
	}
	if(Force && (Sel_CH & 0x10)){
		A5 = FA5;
	}else{
		A5 = get_rtd_val(pVal[4], pVal[8], T5);
	}
	if(Force && (Sel_CH & 0x20)){
		A6 = FA6;
	}else{
		A6 = get_rtd_val(pVal[5], pVal[8], T6);
	}
	if(Force && (Sel_CH & 0x40)){
		A7 = FA7;
	}else{
		A7 = get_rtd_val(pVal[6], pVal[8], T7);
	}
	if(Force && (Sel_CH & 0x80)){
		A8 = FA8;
	}else{
		A8 = get_rtd_val(pVal[7], pVal[8], T8);
	}
