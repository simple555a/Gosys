...
#define RTL_VALID_INTERRUPTS	0x3f	/* All Interrupts above  vicadd */
...
struct rtl81x9_device{
	/* hardware configuration, set by configuration program */
	char * devMemory;
	int ivec; /* hardware irq */
	int ilevel; /* software irq */

	/* software configuration */
	int flags;
	
	/* status and working parameter */
	int nextDesc;
	...
	char * txmemspace;
	int twistie;
	int twist_row, twist_col;
	int counters[8];
	/* interrupt routine */
	void (*on_packet)(void * buffer, int len);

	char last_packet[1518];
	int last_packet_length;
	char * last_sent_packet;
	int last_sent_packet_length;
	
	char _txmemspace[RTL_NUM_TX_DESC * RTL81x9_BUFSIZE];
};

extern struct rtl81x9_device * channel3;

STATUS rtl81x9_write(struct rtl81x9_device * pDrvCtrl, const void * buf, int len);
STATUS rtl81x9_write_ok(struct rtl81x9_device * d);
int rtl81x9_read(struct rtl81x9_device * pDrvCtrl, char * buf, int len, int microseconds);
void rtl81x9_purge(struct rtl81x9_device * d);
STATUS rtl81x9_init(struct rtl81x9_device * pDrvCtrl);
void rtl81x9_uninit(struct rtl81x9_device * pDrvCtrl);
STATUS rtl81x9_start(struct rtl81x9_device * pDrvCtrl);
...
STATUS rtl81x9_reset(struct rtl81x9_device * pDrvCtrl);
STATUS rtl81x9_tune_twister(struct rtl81x9_device * tp);

#define csr_read_byte(d,r,w) *((unsigned char*)(d->devMemory + r))
#define csr_read_word(d,r,w) *((unsigned short*)(d->devMemory + r))
#define csr_read_long(d,r,w) *((unsigned long*)(d->devMemory + r))
#define csr_write_byte(d,r,value,w) *((unsigned char*)(d->devMemory + r)) = value
#define csr_write_word(d,r,value,w) *((unsigned short*)(d->devMemory + r)) = value
#define csr_write_long(d,r,value,w) *((unsigned long*)(d->devMemory + r)) = value
