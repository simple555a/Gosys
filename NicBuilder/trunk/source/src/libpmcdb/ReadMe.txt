...
应用程序向导已为您创建了此 libpmcdb 项目，作为编写动态链接库 (DLL) 
的起点。
...
libpmcdb.idl
    ...
    此文件将由 MIDL 编译器处理以生成：
        C++ 接口定义和 GUID 声明		(libpmcdb.h)
        GUID 定义			(libpmcdb_i.c)
        类型库				(libpmcdb.tlb)
        封送处理代码			(libpmcdb_p.c 和 
                                           dlldata.c）

libpmcdb.h
    此文件包含 libpmcdb.idl 中定义的项目的 C++ 接口定义
...
StdAfx.h，StdAfx.cpp
    这些文件用于生成名为 libpmcdb.pch 的预编译头 (PCH) 文件和
