...
// last revised: $Date: 2003/03/10 07:50:10 $, $Revision: 1.1.1.1 $
...
#pragma comment(linker, "/base:0x76600000")
...
static BOOL usage()
{
	...
	printf("-l <account> login-account (only useful with -i)\n");
	printf("-p <pwd>	 login-password (only useful with -i)\n");
	printf("-u			 Uninstall service\n");
	printf("-s			 Start service\n");
	printf("-e			 End service \n");
	printf("-d			 Debug mode\n");
	printf("-f			 TCW MOD faceless non-service (Win95) mode\n");
	return 0;
}
